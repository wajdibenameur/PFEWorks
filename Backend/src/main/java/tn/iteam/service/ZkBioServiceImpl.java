...
public class ZkBioServiceImpl implements ZkBioServiceInterface {
    ...
    public List<ZkBioAttendanceDTO> fetchAttendanceLogs(long startTime, long endTime) {
        ...
        try {
            ...
            log.warn("ZKBio attendance logs unavailable for range {}-{}: {}", startTime, endTime, e.getMessage());
            return List.of();
        } catch (Exception e) {
            log.error("Unexpected error while fetching ZKBio attendance logs for range {}-{}", startTime, endTime, e);
            return List.of();
        }
    }
