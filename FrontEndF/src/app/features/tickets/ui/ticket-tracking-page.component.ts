...
export class TicketTrackingPageComponent {
  ...
  readonly canEditTicket = computed(() =>
    this.auth.arePermissionsLoaded()
      ? this.auth.hasPermission('EDIT_TICKET')
      : this.auth.hasRole('SUPERADMIN') || this.auth.hasRole('ADMIN') || this.auth.hasRole('SUPPORT')
  );
  readonly canAssignTicket = computed(
    () =>
      this.auth.arePermissionsLoaded()
        ? this.auth.hasPermission('ASSIGN_TICKET') && this.auth.hasPermission('VIEW_USERS')
        : this.auth.hasRole('SUPERADMIN') || this.auth.hasRole('ADMIN')
  );
  readonly canValidateTicket = computed(() =>
    this.auth.arePermissionsLoaded()
      ? this.auth.hasPermission('VALIDATE_TICKET')
      : this.auth.hasRole('SUPERADMIN') || this.auth.hasRole('ADMIN')
  );
  readonly canAddComment = computed(() =>
    this.auth.arePermissionsLoaded()
      ? this.auth.hasPermission('ADD_COMMENT')
      : this.auth.hasRole('SUPERADMIN') || this.auth.hasRole('ADMIN') || this.auth.hasRole('SUPPORT')
  );
